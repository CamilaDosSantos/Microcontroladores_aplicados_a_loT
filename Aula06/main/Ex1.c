...
void Vpot_task(){
    ...
    xQueueSend(ValorPot, &tensao, 0);
    xQueueSend(ValorV, &tensao, 0);
...
void LigaLed_task(){
    ...
    while(1){
        ...
        if(xQueueReceive(ValorV, &liga_led, portMAX_DELAY)){

            if(liga_led < 4.2 && liga_led  > 3.75){
                gpio_set_level(LED_VERDE, 1);
                gpio_set_level(LED_AMARELO, 0);
                gpio_set_level(LED_VERMELHO, 0);
                
            }else if( liga_led  < 3.76 && liga_led  > 3.4){
                gpio_set_level(LED_VERDE, 0);
                gpio_set_level(LED_AMARELO, 1);
                gpio_set_level(LED_VERMELHO, 0);
            

            }else if ( liga_led  < 3.5 && liga_led  > 2.8){
                gpio_set_level(LED_VERDE, 0);
                gpio_set_level(LED_AMARELO, 0);
...
void app_main(void)
{
    ...
    //cria fila
    ValorPot = xQueueCreate(3, sizeof(float));
    ValorV = xQueueCreate(3, sizeof(float));
    ...
    xTaskCreate(LigaLed_task, "led_task", 2048, NULL, 12, NULL);
