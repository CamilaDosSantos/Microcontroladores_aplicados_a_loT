...
{  
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
        //start complete event to iscandicate scan start com sucesso ou falhou
    ...
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        //evento de início completo do adv para indicar o início do adv com sucesso ou falha
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: 
    {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            /* Procure pelo pacote BLE iBeacon */
            if (esp_ble_is_ibeacon_packet(scan_result->scan_rst.ble_adv, scan_result->scan_rst.adv_data_len))
            {
                ...
                double MP =ibeacon_data->ibeacon_vendor.measured_power;
                double RSSI = scan_result->scan_rst.rssi;
                double distancia = (pow(10,(MP - RSSI)/20));
                ESP_LOGI(DEMO_TAG, "Measured power (RSSI at a 1m distance):%d dbm", ibeacon_data->ibeacon_vendor.measured_power);
                ESP_LOGI(DEMO_TAG, "RSSI of packet:%d dbm", scan_result->scan_rst.rssi);
                ESP_LOGI(DEMO_TAG, "distance:%lf m", distancia);
                ...
                ESP_LOGI(DEMO_TAG, "Minor: 0x%04x (%d)", minor, minor);
                        ...
                        if(distancia < 1)//liga led conforme a distancia
                        { 
...
void ble_ibeacon_appRegister(void)
{
    ...
    //registrar a função de retorno de chamada de varredura no módulo de intervalo
