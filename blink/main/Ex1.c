...
static void read_batt_task(void *arg)
{
    ...
    while (1)
    {
        ...
        if ( ( bits & NOTIFY_ENABLED_BIT) && (bits & CONNECTED_BIT) ) 
            {
                if(xQueueReceive(value_voltage, &porcentagem, portMAX_DELAY)) {
                    ESP_LOGI(TAG, "Bateria: %d", porcentagem);
...
void bateria_task(void){
    ...
    while(1){
        bateria = adc1_get_raw(ADC1_CHANNEL_6);
        bateria = calibra_adc(bateria);
        bateria *= 2;
        printf("Valor Bateria: %.2f\n", bateria);

        porcento = (bateria * 100) / 4.2;
        printf("Valor Porcento: %d\n", porcento);

        xQueueSend(value_voltage, &porcento, 0);
...
void app_main(void)
{
    ...
    // cria fila
	value_voltage = xQueueCreate(4, sizeof(int));

    //criando tarefa de envio
    xTaskCreate(read_batt_task, "batt", 4098, NULL, 5, NULL);
    xTaskCreate(bateria_task, "potenciometro_task", 2048, NULL, 12, NULL);
}
